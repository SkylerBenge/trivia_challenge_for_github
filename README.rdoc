...
- The profile information will be discoverable by visitors to the website through the 'details' column in in the high score charts; clicking 'details' in the score row of a particular user will display that user's profile information.
...
- The Profile Information and Leader Board Score Information will be publicly accessible. Additionally, visitors will be able to take a short quiz ranging in length from 1 to 10 questions (at the vistor's discretion) without logging into the website. The quiz results do not get posted to an high score tables.
...
- A "user" has many "scoreboards". A "scoreboard" belongs to a "user". (this name choice is a bit confusing... a scoreboard object is best thought of as an index card holding the score for a single challenge type that is give to an individual users. when the "high score" does it's calculations it uses all the scoreboard objects belonging to all the users that are of the cahallenge type of interest)
